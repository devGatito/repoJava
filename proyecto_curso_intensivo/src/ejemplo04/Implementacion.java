...
public class Implementacion implements metodos {
	...
	public Productos buscarporNombre(String nombre) {
		Productos buscarporNombre = null;
		for (Entry<Integer, Productos> p : hash.entrySet()) {
			if (p.getValue().getNombre().equalsIgnoreCase(nombre)) {
	...
	public Productos buscarSabor(String sabor) {
		Productos buscarSabor = null;

		for (Productos p : hash.values()) {
	...
	public void  eliminarLamnda(String nombre) {
		hash.values().removeIf(a -> a.getNombre().contentEquals(nombre));
